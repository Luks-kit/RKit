use crate::ast::{Expr, ExprKind, ExtendItem, Stmt, StmtKind};
...
impl<'ctx> Compiler<'ctx> {
        ...
        match expr.kind {
            ...
            ExprKind::Variable(name) => {
                ...
                if slot.is_ref {
                    ...
                    self.builder
                        .build_load(pointee_ty, val.into_pointer_value(), &name)
                        .map_err(|e| e.to_string())
            ...
            ExprKind::Cast { target_type, expr } => {
                ...
                match (val, target_ty) {
                    ...
                    // int -> ptr
                    (BasicValueEnum::IntValue(i), BasicTypeEnum::PointerType(t)) => Ok(self
                        .builder
                        .build_int_to_ptr(i, t, "cast")
                        .map_err(|e| e.to_string())?
                        .as_basic_value_enum()),
                    // ptr -> int
                    (BasicValueEnum::PointerValue(p), BasicTypeEnum::IntType(t)) => Ok(self
                        .builder
                        .build_ptr_to_int(p, t, "cast")
                        .map_err(|e| e.to_string())?
                        .as_basic_value_enum()),
                    // int -> int (truncate or extend)
                    (BasicValueEnum::IntValue(i), BasicTypeEnum::IntType(t)) => Ok(self
                        .builder
                        .build_int_cast(i, t, "cast")
                        .map_err(|e| e.to_string())?
                        .as_basic_value_enum()),
                    // float -> int
                    (BasicValueEnum::FloatValue(f), BasicTypeEnum::IntType(t)) => Ok(self
                        .builder
                        .build_float_to_signed_int(f, t, "cast")
                        .map_err(|e| e.to_string())?
                        .as_basic_value_enum()),
                    // int -> float
                    (BasicValueEnum::IntValue(i), BasicTypeEnum::FloatType(t)) => Ok(self
                        .builder
                        .build_signed_int_to_float(i, t, "cast")
                        .map_err(|e| e.to_string())?
                        .as_basic_value_enum()),
            ...
            ExprKind::Call { callee, args } => {
                ...
                if let Some(extend) = self.extends.get(&callee_name).cloned() {
                    if let Some(init_name) = extend.init_fn {
                        ...
                        let call = self
                            .builder
                            .build_call(function, &compiled_args, "initcall")
                            .map_err(|e| e.to_string())?;
            ...
            ExprKind::StructInit { name, fields } => {
                ...
                for (i, (_, val_expr)) in fields.iter().enumerate() {
                    ...
                    let ptr = self
                        .builder
                        .build_struct_gep(struct_ty, alloca, i as u32, "field")
                        .map_err(|e| e.to_string())?;
                    self.builder
                        .build_store(ptr, val)
                        .map_err(|e| e.to_string())?;
                ...
                Ok(self
                    .builder
                    .build_load(struct_ty, alloca, &name)
                    .map_err(|e| e.to_string())?)
            }

            ExprKind::FieldAccess { object, field } => {
                let vname = match object.kind {
                    ExprKind::Variable(ref vname) => vname.clone(),
                ...
                let idx = def
                    .field_index(&field)
                    .ok_or_else(|| format!("No field '{}' on '{}'", field, struct_name))?
                    as u32;
                ...
                let field_ptr = self
                    .builder
                    .build_struct_gep(struct_ty, struct_ptr, idx, &field)
                    .map_err(|e| e.to_string())?;
                self.builder
            ...
            ExprKind::SliceLiteral(elements) => {
                ...
                let compiled: Vec<BasicValueEnum> = elements
                    .into_iter()
                ...
                self.builder
                    .build_load(array_ty, alloca, "slice_val")
                    .map_err(|e| e.to_string())
            }
            ...
            ExprKind::Index { object, index } => {
                ...
                let slot = self
                    .lookup_var(&name)
                    .ok_or_else(|| format!("Undefined variable '{}'", name))?;
                let is_const_index = matches!(index.kind, ExprKind::Literal(Value::Int(_)));
                        ...
                        let ptr = unsafe {
                            self.builder
                                .build_gep(
                                    slot.ty,
                                    slot.ptr,
                                    &[self.context.i64_type().const_int(0, false), idx_val],
                                    "elem_ptr",
                                )
                                .map_err(|e| e.to_string())?
                        };
            ...
            ExprKind::Len(expr) => {
                ...
                let slot = self
                    ...
                    .ok_or_else(|| format!("Undefined variable '{}'", name))?;
                match slot.ty {
                    BasicTypeEnum::ArrayType(arr_ty) => Ok(self
                        .context
                        .i64_type()
                        .const_int(arr_ty.len() as u64, false)
                        .as_basic_value_enum()),
                    _ => Err(format!(
                        "len() requires a slice, '{}' is not an array",
                        name
                    )),
                }
            }
            ...
            ExprKind::MethodCall {
                ...
                if self.modules.contains(&var_name) {
                    ...
                    let call = self
                        .builder
                        .build_call(function, &compiled_args, "modcall")
                        .map_err(|e| e.to_string())?;
                    return match call.try_as_basic_value() {
                        ValueKind::Basic(val) => Ok(val),
                        ValueKind::Instruction(_) => Ok(self
                ...
                let call = self
                    .builder
                    .build_call(function, &compiled_args, "methodcall")
                    .map_err(|e| e.to_string())?;
                match call.try_as_basic_value() {
                    ValueKind::Basic(val) => Ok(val),
                    ValueKind::Instruction(_) => Ok(self
        ...
        match stmt.kind {
            StmtKind::VarDecl {
                ...
                let alloca = self
                    ...
                    .map_err(|e| e.to_string())?;
                self.builder
                    .build_store(alloca, initial_val)
                    .map_err(|e| e.to_string())?;
                self.define_var(
                    name,
                    VarSlot {
            ...
            StmtKind::Return(expr) if matches!(expr.kind, ExprKind::Literal(Value::Null)) => {
                let scopes: Vec<Vec<(String, PointerValue<'ctx>)>> = self
                    .variables
                    .iter()
                    .map(|scope| {
                        scope
                            ...
                            Some(ty) => {
                                let zero = match ty {
                                    ...
                                    BasicTypeEnum::FloatType(t) => {
                                        t.const_float(0.0).as_basic_value_enum()
                                    }
                                    other => {
                                        return Err(format!(
                                            "Cannot emit fallback return for type {:?}",
                                            other
                                        ))
                                    }
                                };
            ...
            StmtKind::Struct { name, fields } => {
                let typed_fields: Vec<(String, LKitType)> = fields
                    .iter()
            ...
            StmtKind::LetDecl { .. } => unreachable!("LetDecl should be folded by type checker"),
            StmtKind::Import { .. } => Ok(()),
            StmtKind::Tool { .. } => Ok(()), // no codegen needed
            ...
            StmtKind::ExtendWith {
                ...
                // compile exactly like StmtKind::Extend methods
        ...
        for stmt in stmts {
            ...
            match stmt.kind {
                StmtKind::Function {
                    ...
                    self.compile_statement(Stmt::new(
                        StmtKind::Function {
                            name: mangled,
                            params,
                            return_type,
                ...
                other => self.compile_statement(Stmt::new(other, span.clone()))?,
            }
        }
        Ok(())
    }
    ...
    fn type_str_to_llvm(&self, ty: &str) -> BasicTypeEnum<'ctx> {
        match ty {
            ...
            other if other.ends_with('&') => self.context.ptr_type(AddressSpace::default()).into(),
    ...
    fn emit_dinits_for_vars(
        &mut self,
        vars: &[(String, PointerValue<'ctx>)],
    ) -> Result<(), String> {
            ...
            if let Some(extend) = self.extends.get(&struct_name).cloned() {
                if let Some(dinit_name) = extend.dinit_fn {
                    if let Some(dinit_fn) = self.module.get_function(&dinit_name) {
                        ...
                        self.builder
                            .build_call(dinit_fn, &[target.into()], "")
                            .map_err(|e| e.to_string())?;
                    }
                }
            }
